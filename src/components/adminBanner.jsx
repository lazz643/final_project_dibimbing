...
import { FaArrowLeft, FaPlus, FaEdit, FaTrash, FaChevronLeft, FaChevronRight, FaSearch, FaTimes, FaBullhorn, FaEye, FaImage } from "react-icons/fa";
...
function AdminBanner() {
  ...
  const handleCreateBanner = async () => {
    try {
      ...
      if (error.response?.data?.message) {
        ...
        setFormErrors({ general: "Failed to create banner" });
      }
    } finally {
  ...
  const handleUpdateBanner = async () => {
    try {
      ...
      await axios.post(`https://travel-journal-api-bootcamp.do.dibimbing.id/api/v1/update-banner/${selectedBanner.id}`, formData, {
        headers: {
          apiKey: "24405e01-fbc1-45a5-9f5a-be13afcd757c",
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
      ...
      if (error.response?.data?.message) {
        ...
        setFormErrors({ general: "Failed to update banner" });
      }
    } finally {
      setSubmitLoading(false);
    }
  ...
  const handleDeleteBanner = async (bannerId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`https://travel-journal-api-bootcamp.do.dibimbing.id/api/v1/delete-banner/${bannerId}`, {
        headers: {
          apiKey: "24405e01-fbc1-45a5-9f5a-be13afcd757c",
          Authorization: `Bearer ${token}`,
        },
  ...
  const filteredBanners = banners.filter((banner) => banner.name.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const handleBannersPerPageChange = (e) => {
    setBannersPerPage(parseInt(e.target.value));
    setCurrentPage(1);
  };
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                  <FaBullhorn className="text-[#28cdba]" />
                  Manage Banners
                </h1>
                <p className="text-gray-600 mt-1">Update promotional banners</p>
              </div>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <select value={bannersPerPage} onChange={handleBannersPerPageChange} className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#28cdba] focus:border-transparent">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center justify-between">
              ...
              <div className="p-3 bg-orange-100 rounded-lg">
                <FaBullhorn className="text-orange-600" size={24} />
              </div>
            </div>
          </div>
          <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{filteredBanners.length}</p>
              </div>
                ...
                {currentBanners.map((banner) => (
                  <tr key={banner.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="text-sm text-gray-500">ID: {banner.id.slice(0, 8)}...</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDate(banner.createdAt)}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDate(banner.updatedAt)}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center space-x-2">
                        <button onClick={() => openEditModal(banner)} className="text-[#28cdba] hover:text-[#20b5a5] p-1 rounded transition-colors" title="Edit">
                          <FaEdit size={16} />
                        </button>
                        <button onClick={() => handleDeleteBanner(banner.id)} className="text-red-600 hover:text-red-800 p-1 rounded transition-colors" title="Delete">
                          <FaTrash size={16} />
                        </button>
                      </div>
                    </td>
        ...
        <div className="md:hidden space-y-4">
          {currentBanners.map((banner) => (
            <div key={banner.id} className="bg-white rounded-lg shadow-sm p-4">
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 mt-1">ID: {banner.id.slice(0, 8)}...</p>
                  ...
                  <div className="flex items-center justify-end space-x-2 mt-3">
                    <button onClick={() => openEditModal(banner)} className="text-[#28cdba] hover:text-[#20b5a5] p-2 rounded transition-colors" title="Edit">
                      <FaEdit size={16} />
                    </button>
                    <button onClick={() => handleDeleteBanner(banner.id)} className="text-red-600 hover:text-red-800 p-2 rounded transition-colors" title="Delete">
                      <FaTrash size={16} />
                    </button>
                  </div>
                </div>
        ...
        {filteredBanners.length === 0 && !loading && (
          <div className="bg-white rounded-lg shadow-sm p-12 text-center">
            ...
            <p className="mt-2 text-gray-500">{searchTerm ? "Try adjusting your search terms" : "Get started by creating your first banner"}</p>
            {!searchTerm && (
            ...
            <div className="px-4 md:px-6 py-4 border-t border-gray-200">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="text-[12px] text-gray-700 order-2 sm:order-1">
                  Showing {(currentPage - 1) * bannersPerPage + 1} to {Math.min(currentPage * bannersPerPage, filteredBanners.length)} of {filteredBanners.length} banners
                </div>
          ...
          <div className="bg-white rounded-lg max-w-lg w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-gray-900">{modalMode === "create" ? "Create New Banner" : "Edit Banner"}</h2>
